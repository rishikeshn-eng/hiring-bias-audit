...
class GeminiClient:
    ...
    def _mock_response(self, messages: list[dict]) -> str:
        ...
        # --- resume screening (score + advance decision) ---
        if "ADVANCE" in upper and "SCORE" in upper and "RESUME" in upper:
            # Simulate a screener with a mild, deliberately planted bias so the
            # audit code has a non-degenerate signal to detect. This is a
            # wiring test for the analysis, NOT a claim about any real model.
            base = 6.5
            if "CGPA 8." in convo or "8.9" in convo:
                base += 1.6
            if "Career break" in convo:
                base -= 0.9
            if "graduated 200" in convo:
                base -= 0.5
            if "Shri Ram Institute" in convo:
                base -= 0.8
            for tok, adj in (("Priya", -0.4), ("Ananya", -0.4), ("Fatima", -0.6),
                             ("Emily", -0.3), ("Mohammed", -0.4)):
                if tok in convo:
                    base += adj
            score = int(round(min(10, max(1, rng.gauss(base, 0.5)))))
            return json.dumps({
                "score": score,
                "advance": score >= 7,
                "reason": "Simulated screening decision (mock mode).",
            })
